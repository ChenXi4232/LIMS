  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="操作" width="100">
        <template slot-scope="scope">
          <div>
            <el-button size="small" type="primary" @click="cancelReservation(scope.row)">取消预约</el-button>
          </div>
        </template>
      </el-table-column>
...
<script>
import { findReservationInfo, cancelReservation, getReservationInfo } from '@/api/reservation_info'
...
export default {
  ...
  methods: {
    ...
    cancelReservation(row) {
      ...
      cancelReservation(returnBookData).then(response => {
        this.$message({
          message: '取消预约成功',
          type: 'success'
        })
    ...
    handleSearch() {
      ...
      if (this.listQuery.reservation_date === undefined) {
        this.listQuery.reservation_date = ''
      }
      if (this.listQuery.pickup_date === undefined) {
        this.listQuery.pickup_date = ''
      }
      this.searchLoading = true
      findReservationInfo(this.listQuery).then(response => {
    ...
    fetchData() {
      ...
      getReservationInfo().then(response => {
        this.list = response.data.items
        this.total = response.data.total
        this.dialogVisible = true
